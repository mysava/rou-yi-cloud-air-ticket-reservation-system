<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="出发机场" prop="departportname">
        <el-input
          v-model="queryParams.departportname"
          placeholder="请输入出发机场"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:flights:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:flights:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:flights:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="flightsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="出发日期" align="center" prop="departdate" />
      ...
      <el-table-column label="出发机场" align="center" prop="departportname" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="出发日期" prop="departdate">
          <el-input v-model="form.departdate" placeholder="请输入出发日期" />
        </el-form-item>
        <el-form-item label="出发时间" prop="departtime">
        ...
        <el-form-item label="出发机场" prop="departportname">
          <el-input v-model="form.departportname" placeholder="请输入出发机场" />
        </el-form-item>
        <el-form-item label="到达城市" prop="arrivecityname">
...
<script>
import { listFlights, getFlights, delFlights, addFlights, updateFlights } from "@/api/system/flights";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.flightno)
    ...
    handleUpdate(row) {
      this.reset();
      const flightno = row.flightno || this.ids
      getFlights(flightno).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const flightnos = row.flightno || this.ids;
      this.$modal.confirm('是否确认删除航班信息管理编号为"' + flightnos + '"的数据项？').then(function() {
        return delFlights(flightnos);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/flights/export', {
        ...this.queryParams
