...
<!--      </el-form-item>-->
    ...
    <el-table v-loading="loading" :data="amendList" @selection-change="handleSelectionChange">
...
<!--      <el-table-column label="普通舱数量" align="center" prop="comNum" />-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="订单编号" prop="ticketId">
          <el-input v-model="form.ticketId" />
        </el-form-item>
        <el-form-item label="编号" prop="flightid">
          <el-input v-model="form.flightid" />
        </el-form-item>
...
<script>
import { listAmend, getAmend, delAmend, addAmend, updateAmend } from "@/api/system/amend";
import {alterTickets} from "@/api/system/tickets";
...
export default {
  ...
  created() {
    // 从路由中取参数
    this.ticketId = this.$route.query.id||null;
    // 赋值给查询参数
    this.queryParams.ticketId = this.ticketId;
    this.getList();
  },
  watch:{
    $route() {
      this.ticketId = this.$route.query && this.$route.query.id;
    },
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAmend(flightid).then(response => {
    ...
    reserve() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          alterTickets(this.form,ticketId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除订单修改编号为"' + flightids + '"的数据项？').then(function() {
        return delAmend(flightids);
      }).then(() => {
        this.getList();
